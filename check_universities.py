...
# already checked and could not find github organization
skip = set([
    'abcnash.edu',
    'selmauniversity.edu', # no such domain
    'swcc.edu',
    'vuu.edu',
])
...
def main():
    organizations = get_all_the_domains_we_already_have()
    ...
    with open(filename) as fh:
        ...
        for univ in universities:
            for domain in univ['domains']:
                if domain in skip:
                    continue
                if not domain in organizations:
                    print(domain)
